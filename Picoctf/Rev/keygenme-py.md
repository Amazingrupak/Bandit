...
```
noticing the check key part, I wrote a rudminetary script to decode what the letters are. Obviously, the sha decoding part is part of the flag as the rest of it checked before entering that if condition.
```
import hashlib
for i in range (9):
    x = hashlib.sha256(b"GOUGH").hexdigest()[i]
    print(x)
```
this prints what each of the if condition returns. And then I just order them in the right order and get the answer. :D 